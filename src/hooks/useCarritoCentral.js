...
export default function useCarritoCentral() {
  ...
  const cantidadDe = (modelo, gusto) => {
    const item = items.find((i) => i.id === itemId(modelo, gusto));
    return item?.qty ?? 0;
  };

  const totalItems = items.reduce((s, i) => s + i.qty, 0);

  return { items, agregar, cambiar, quitar, vaciar, cantidadDe, totalItems };
}
